...
recoil.atom({ key: "initkvAtom", default: {} });
...
const ValueList = ({ subject, property, embedded, force, editable, owner, topEntity, shape, siblingsPath, setCssClass, config }) => {
  ...
  const renderListElem = React.useMemo(() => (val, i, nbvalues) => {
...
function EntityEditContainerMayUpdate(props) {
  ...
  const { copy } = queryString__default["default"].parse(props.location.search, { decode: false });
  React.useEffect(() => {
...
function EntityEditContainer(props, config) {
  ...
  const possiblePrefLabels = recoil.useRecoilValue(canPushPrefLabelGroups ? possiblePrefLabelsSelector({ canPushPrefLabelGroups }) : initMapAtom);
